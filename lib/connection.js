...
Connection.prototype.connect = function (port, host) {
  ...
  this.stream.on('connect', function () {
    global.__KNEXBADEBUG.log(this._connID, "pg ev connect");
    if (self._keepAlive) {
  ...
  this.stream.on('close', function () {
    global.__KNEXBADEBUG.log(this._connID, "pg ev close");
    self.emit('end')
  })
...
Connection.prototype.attachListeners = function (stream) {
  ...
  stream.on('data', function (buff) {
    global.__KNEXBADEBUG.log(self._connID, "pg ev data " + buff.length);
  ...
  stream.on('end', function () {
    global.__KNEXBADEBUG.log(this._connID, "pg ev end");
    self.emit('end')
  })
}
...
Connection.prototype.query = function (text) {
  global.__KNEXBADEBUG.log(this._connID, "pg conn query " + text);
  // 0x51 = Q
